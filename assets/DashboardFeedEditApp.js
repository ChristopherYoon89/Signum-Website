...
const SelectInclCategory = ({ feedinclcategory, categoryoptions, onChangeInclCategory }) => {
	return(
		<>
			<div className="sig-form-header">
				Include categories
				<span>
					<Tooltip
						placement='right'
						title='Select one or more categories to include categories into your feed. 
						If none are selected, articles from all categories will be included'
					>
					<QuestionCircleOutlined className="sig-form-info-icon"	/>
					</Tooltip>
				</span> 
		...
		<>
			<div className="sig-form-header">
				...
				<span>
					<Tooltip
						placement='right'
						title='Enter keywords to include articles. For multiple keywords, separate 
...
const SelectInclSource = ({ feedinclsourcesdisabled, sourcesoptions, feedinclsource, onChangeInclSource }) => {
	return(
		<>
			<div className="sig-form-header">
				Include sources
				<span>
					<Tooltip
						placement='right'
						title='Select one or more sources that should be included into your feed. If none
						are selected, articles from all sources will be included. You can either include 
		...
		<>
			<div className="sig-form-header">
				...
				<span>
					<Tooltip
						placement='right'
						title='Select sources to exclude from your feed. If none are excluded, articles 
						from all sources will be searched. You can either include sources or exclude 
						them, but not both.'
					>
					<QuestionCircleOutlined className="sig-form-info-icon"	/>
					</Tooltip>
...
const DashboardFeedEditApp = () => {
	...
	const fetchSingleFeed = async (feed_id) => {
		try {
			const response = await axios.get(`/api/FeedSingle/${feed_id}/`, {
			...
			// Category select (JSONField returns an array)
			setFeedInclCategory(feed.category_included || []);

			// Tags included/excluded
			if (feed.tags_included && feed.tags_included.length > 0) {
				setFeedInclKeywords(feed.tags_included.join(' '));
			...
			if (feed.tags_excluded && feed.tags_excluded.length > 0) {
					setFeedExclKeywords(feed.tags_excluded.join(' '));
			...
			const includedSources = Array.isArray(feed.source_included)
				? feed.source_included
				: [];

			setFeedInclSource(includedSources);

			// disable excluded sources only if included sources exist
			setFeedExclSourcesDisabled(includedSources.length > 0);


			// Sources excluded
			const excludedSources = Array.isArray(feed.source_excluded)
				? feed.source_excluded
				: [];

			setFeedExclSource(excludedSources);

			// disable included sources only if excluded sources exist
			setFeedInclSourcesDisabled(excludedSources.length > 0);

			// Numbers
			setFeedMinClicks(feed.min_clicks || 0);
			setFeedMinUserRating(feed.min_rating || 0);
			setFeedMinAlgoRating(feed.min_algo_rating || 0);
			setFeedMinSourceRating(feed.min_source_rating || 0);
			} catch (error) {
	...
	const handleUpdateFeed = async () => {
		...
		try {
			const response = await axios.patch(`/api/FeedSingle/${feed_id}/`,
			{
...
export default DashboardFeedEditApp;
