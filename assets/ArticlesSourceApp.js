...
const ArticlesSourceApp = () => {
  ...
  const [loading, setloading] = useState(true);
	...
	const source_name = decodeURIComponent(sourceName)
  ...
  useEffect(() => {
	...
	const getData = async (pageNumber = 1) => {
		try {
			...
			const response = await axios.get(`/api/NewsSourceSingle/?source=${encodeURIComponent(source_name)}&page=${pageNumber}`);
