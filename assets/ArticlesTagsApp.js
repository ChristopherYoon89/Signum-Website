...
const ArticlesTagsApp = () => {
  ...
  const [userbookmarks, setUserBookmarks] = useState([]);
	...
	const tag_name = decodeURIComponent(tagName)
	...
	const getData = async (pageNumber = 1) => {
		try {
			...
			const response = await Axios.get(`/api/NewsTagsSingle/?tag=${encodeURIComponent(tag_name)}&page=${pageNumber}`);
...
export default ArticlesTagsApp; 