...
const ArticlesCategoryApp = () => {
  ...
  const [userbookmarks, setUserBookmarks] = useState([]);
	...
	const category_name = decodeURIComponent(categoryName)
	...
	const getData = async (pageNumber = 1) => {
		try {
			...
			const response = await Axios.get(`/api/ArticlesByCategory/?category_primary=${encodeURIComponent(category_name)}&page=${pageNumber}`);
...
export default ArticlesCategoryApp; 